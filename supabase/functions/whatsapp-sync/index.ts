  ...
  try {
    ...
    for (const chat of topChats) {
      ...
      const { data: existingMsgs } = await supabase
        ...
        .select('external_message_id, raw_payload')
        .eq('conversation_id', conv.id);
      
      const existingIds = new Set(existingMsgs?.map((m: any) => m.external_message_id || m.raw_payload?.key?.id || m.raw_payload?.id).filter(Boolean));
      ...
      while (hasMore) {
        ...
        for (const m of msgs) {
          ...
          const messageId = m.key?.id || m.id || null;
        ...
        if (messagesToInsert.length > 0) {
          const { error: upsertError } = await supabase
            .from('whatsapp_messages')
            .upsert(messagesToInsert, { onConflict: 'conversation_id,external_message_id' });
          if (!upsertError) {
             syncedMessages += messagesToInsert.length;
             for (const m of messagesToInsert) {
               const msgId = m.external_message_id || m.raw_payload?.key?.id || m.raw_payload?.id;
